...
import {
  Building2, AlertTriangle, Clock, CheckCircle, Package, TrendingUp, Users,
  X, Loader2, Phone, MapPin, Calendar, Truck
} from "lucide-react";
...
export default function MilkHospitalDashboard({
  ...
  const handleSchedulePickup = async () => {
    ...
    setIsUpdating(true);
    try {
      // Update match status to pickup_scheduled
      await api.milk.updateMatchStatus(selectedMatch.id, {
        status: "pickup_scheduled",
        pickup_date: pickupDate,
        pickup_time: pickupTime,
      });
      toast.success("Pickup scheduled! Donor will be notified.");
  ...
  const handleLogDonation = async (match: any) => {
    ...
    try {
      ...
      const result = await api.milk.createDonation({
        donor_id: match.donor_id || match.id,
      ...
      // Update match status to delivered
      await api.milk.updateMatchStatus(match.id, { status: "delivered" });

      toast.success(`Donation logged! Passport: ${result.passport_id}`);
      onRefresh();
    } catch (e: any) {
  ...
  const getStatusConfig = (status: string) => {
    return MATCH_STATUSES[status as keyof typeof MATCH_STATUSES] || MATCH_STATUSES.pending;
  };
  ...
  return (
    <div className="space-y-8">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
        ...
        {/* Matched Donors with Workflow Actions */}
        <div className="rounded-2xl border bg-card p-6 shadow-card">
          ...
          {dashboard.matched_donors.length === 0 ? (
            ...
            <div className="space-y-3">
              {dashboard.matched_donors.map((m) => {
                  ...
                  <div key={m.id} className="p-4 rounded-xl bg-muted/30 border">
                    ...
                    <div className="flex gap-2 mt-3">
                      {m.status === "accepted" && (
                        ...
                        >
                          <Calendar className="w-3 h-3 mr-1" /> Schedule Pickup
                        </Button>
                      )}
            ...
            >
              ...
              <div className="p-6 space-y-4">
                ...
                <p className="font-body text-[11px] text-muted-foreground italic bg-milk/10 p-3 rounded-xl">
                  The donor will receive an SMS and in-app notification with pickup details.
                </p>

                <Button
                  onClick={handleSchedulePickup}
                  disabled={isUpdating || !pickupDate}
